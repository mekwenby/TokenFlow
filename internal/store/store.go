...
const (
	AuthSessionOwnerConsumer = "consumer"
	AuthSessionOwnerAdmin    = "admin"
)
...
const (
	...
	ChatMessageStatusGenerating = "generating"
...
type ChatConversation struct {
	...
	ContextSummaryThroughMessageID int64      `json:"-"`
}
...
func (s *Store) Migrate(ctx context.Context) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS chat_conversations (
			...
			context_summary_through_message_id INTEGER NOT NULL DEFAULT 0,
			CHECK(
				(owner_type = 'consumer' AND consumer_user_id IS NOT NULL AND admin_user_id IS NULL) OR
				(owner_type = 'admin' AND admin_user_id IS NOT NULL AND consumer_user_id IS NULL)
			)
...
func (s *Store) ensureChatMessageLifecycleColumns(ctx context.Context) error {
	rows, err := s.db.QueryContext(ctx, `PRAGMA table_info(chat_messages)`)
	if err != nil {
		return err
	}
	...
	if !columns["parent_message_id"] {
		if _, err := s.db.ExecContext(ctx, `ALTER TABLE chat_messages ADD COLUMN parent_message_id INTEGER REFERENCES chat_messages(id) ON DELETE CASCADE`); err != nil {
			return err
		}
	}
	if !columns["request_id"] {
		if _, err := s.db.ExecContext(ctx, `ALTER TABLE chat_messages ADD COLUMN request_id TEXT NOT NULL DEFAULT ''`); err != nil {
			return err
		}
	}
	if !columns["status"] {
		if _, err := s.db.ExecContext(ctx, `ALTER TABLE chat_messages ADD COLUMN status TEXT NOT NULL DEFAULT 'completed'`); err != nil {
			return err
		}
	}
	if _, err := s.db.ExecContext(ctx, `UPDATE chat_messages
		SET status = CASE
			WHEN json_extract(metadata, '$.completion_status') = 'stopped' THEN 'stopped'
			WHEN json_extract(metadata, '$.completion_status') IN ('failed', 'stream_failed') THEN 'failed'
			ELSE status
		END
		WHERE role = 'assistant' AND json_valid(metadata)`); err != nil {
		return err
	}
	if _, err := s.db.ExecContext(ctx, `UPDATE chat_messages AS assistant
		SET parent_message_id = (
			SELECT user.id FROM chat_messages AS user
			WHERE user.conversation_id = assistant.conversation_id AND user.role = 'user' AND user.id < assistant.id
			ORDER BY user.id DESC LIMIT 1
		)
		WHERE assistant.role = 'assistant' AND assistant.parent_message_id IS NULL`); err != nil {
		return err
	}
	if _, err := s.db.ExecContext(ctx, `INSERT INTO chat_messages(conversation_id, parent_message_id, role, content, metadata, status)
		SELECT conversation.id, last_message.id, 'assistant', '', '{}',
			CASE WHEN conversation.status = 'stopped' THEN 'stopped' ELSE 'failed' END
		FROM chat_conversations AS conversation
		JOIN chat_messages AS last_message ON last_message.id = (
			SELECT candidate.id FROM chat_messages AS candidate
			WHERE candidate.conversation_id = conversation.id ORDER BY candidate.id DESC LIMIT 1
		)
		WHERE conversation.status IN ('failed', 'stopped')
			AND COALESCE(conversation.active_operation, '') = ''
			AND last_message.role = 'user'`); err != nil {
		return err
	}
	_, err = s.db.ExecContext(ctx, `CREATE UNIQUE INDEX IF NOT EXISTS idx_chat_messages_request ON chat_messages(conversation_id, request_id) WHERE request_id <> ''`)
	return err
}
...
func (s *Store) ensureChatConversationSettingsColumns(ctx context.Context) error {
	...
	if !columns["context_summary"] {
		if _, err := s.db.ExecContext(ctx, `ALTER TABLE chat_conversations ADD COLUMN context_summary TEXT NOT NULL DEFAULT ''`); err != nil {
			return err
		}
	}
	if !columns["context_summary_through_message_id"] {
		if _, err := s.db.ExecContext(ctx, `ALTER TABLE chat_conversations ADD COLUMN context_summary_through_message_id INTEGER NOT NULL DEFAULT 0`); err != nil {
			return err
		}
	}
	return nil
...
func (s *Store) ensureRequestLogCacheColumns(ctx context.Context) error {
	...
	for name, statement := range map[string]string{
		...
		"usage_estimated":   `ALTER TABLE request_logs ADD COLUMN usage_estimated INTEGER NOT NULL DEFAULT 0`,
		"completion_status": `ALTER TABLE request_logs ADD COLUMN completion_status TEXT NOT NULL DEFAULT ''`,
		"request_type":      `ALTER TABLE request_logs ADD COLUMN request_type TEXT NOT NULL DEFAULT ''`,
		"tool_calls":        `ALTER TABLE request_logs ADD COLUMN tool_calls INTEGER NOT NULL DEFAULT 0`,
	} {
		if !columns[name] {
...
func (s *Store) CreateAuthSession(ctx context.Context, session AuthSession) error {
	createdAt := formatAuthSessionTime(session.CreatedAt)
	expiresAt := formatAuthSessionTime(session.ExpiresAt)
	var err error
	switch session.OwnerType {
	case AuthSessionOwnerConsumer:
		_, err = s.db.ExecContext(ctx, `INSERT INTO auth_sessions(
			owner_type, consumer_user_id, token_hash, csrf_token_hash, created_at, expires_at
		) VALUES(?, ?, ?, ?, ?, ?)`, session.OwnerType, session.UserID, session.TokenHash, session.CSRFTokenHash, createdAt, expiresAt)
	case AuthSessionOwnerAdmin:
		_, err = s.db.ExecContext(ctx, `INSERT INTO auth_sessions(
			owner_type, admin_user_id, token_hash, csrf_token_hash, created_at, expires_at
		) VALUES(?, ?, ?, ?, ?, ?)`, session.OwnerType, session.UserID, session.TokenHash, session.CSRFTokenHash, createdAt, expiresAt)
	default:
		return fmt.Errorf("invalid auth session owner type %q", session.OwnerType)
	}
	return err
}

func (s *Store) AuthSessionByTokenHash(ctx context.Context, ownerType, tokenHash string) (AuthSession, error) {
	var session AuthSession
	var createdAt, expiresAt string
	err := s.db.QueryRowContext(ctx, `SELECT
		s.id,
		s.owner_type,
		CASE WHEN s.owner_type = 'consumer' THEN s.consumer_user_id ELSE s.admin_user_id END,
		COALESCE(c.email, a.username, ''),
		COALESCE(c.status, ''),
		s.token_hash,
		s.csrf_token_hash,
		s.created_at,
		...
		LEFT JOIN consumer_users c ON c.id = s.consumer_user_id
		LEFT JOIN admin_users a ON a.id = s.admin_user_id
		WHERE s.owner_type = ? AND s.token_hash = ?`, ownerType, tokenHash).Scan(
		&session.ID,
		&session.OwnerType,
		&session.UserID,
		&session.Username,
...
func (s *Store) RenewAuthSession(ctx context.Context, ownerType, tokenHash, csrfTokenHash string, now, expiresAt time.Time) error {
	res, err := s.db.ExecContext(ctx, `UPDATE auth_sessions
		SET csrf_token_hash = ?, expires_at = ?
		WHERE owner_type = ? AND token_hash = ? AND expires_at > ?`,
		csrfTokenHash,
...
func (s *Store) DeleteAuthSessionByTokenHash(ctx context.Context, ownerType, tokenHash string) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM auth_sessions WHERE owner_type = ? AND token_hash = ?`, ownerType, tokenHash)
	return err
}

func (s *Store) RevokeAuthSessions(ctx context.Context, ownerType string, userID int64) error {
	var err error
	switch ownerType {
	case AuthSessionOwnerConsumer:
		_, err = s.db.ExecContext(ctx, `DELETE FROM auth_sessions WHERE owner_type = ? AND consumer_user_id = ?`, ownerType, userID)
	case AuthSessionOwnerAdmin:
		_, err = s.db.ExecContext(ctx, `DELETE FROM auth_sessions WHERE owner_type = ? AND admin_user_id = ?`, ownerType, userID)
	default:
		return fmt.Errorf("invalid auth session owner type %q", ownerType)
	}
	return err
}

func (s *Store) DeleteExpiredAuthSessions(ctx context.Context, now time.Time) error {
	_, err := s.db.ExecContext(ctx, `DELETE FROM auth_sessions WHERE expires_at <= ?`, formatAuthSessionTime(now))
	return err
}
...
func (s *Store) UpdateConsumerUser(ctx context.Context, id int64, status string, quotaTotalTokens int64) (ConsumerUser, error) {
	...
	defer tx.Rollback()
	if _, err := tx.ExecContext(ctx, `UPDATE consumer_users SET status = ?, quota_total_tokens = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, status, quotaTotalTokens, id); err != nil {
		return ConsumerUser{}, err
	}
...
func (s *Store) AvailableChatModels(ctx context.Context) ([]string, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT m.client_model
		FROM model_mappings m
		JOIN providers p ON p.id = m.provider_id
		WHERE p.enabled = 1
		ORDER BY m.client_model ASC`)
	if err != nil {
		return nil, err
	}
	seen := map[string]bool{}
...
func (s *Store) ResolveChatRoute(ctx context.Context, clientModel string) (Route, error) {
	clientModel = strings.TrimSpace(clientModel)
	if clientModel == "" {
		return Route{}, ErrNotFound
	}
	route, err := s.ResolveRoute(ctx, clientModel)
	if err != nil {
		return Route{}, err
	}
	...
	var mapped int
	err = s.db.QueryRowContext(ctx, `SELECT 1 FROM model_mappings m JOIN providers p ON p.id = m.provider_id WHERE m.client_model = ? AND p.enabled = 1`, clientModel).Scan(&mapped)
	if err == nil {
		return route, nil
	}
	if !errors.Is(err, sql.ErrNoRows) {
...
func (s *Store) InsertRequestLog(ctx context.Context, log RequestLog) error {
	...
	_, err := s.db.ExecContext(ctx, `INSERT INTO request_logs(protocol, model, upstream_model, provider_id, distribution_key_id, distribution_key_name, consumer_user_id, consumer_user_email, admin_user_id, admin_username, status_code, latency_ms, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens, stream, billable, usage_estimated, completion_status, request_type, tool_calls)
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		log.Protocol, log.Model, log.UpstreamModel, providerID, distributionKeyID, log.DistributionKeyName, consumerUserID, log.ConsumerEmail, adminUserID, log.AdminUsername, log.StatusCode, log.LatencyMS, log.InputTokens, log.OutputTokens, log.CacheReadTokens, log.CacheCreationTokens, boolInt(log.Stream), boolInt(log.Billable), boolInt(log.UsageEstimated), log.CompletionStatus, log.RequestType, log.ToolCalls)
	return err
}
...
func (s *Store) RecordRequest(ctx context.Context, log RequestLog) error {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO request_logs(protocol, model, upstream_model, provider_id, distribution_key_id, distribution_key_name, consumer_user_id, consumer_user_email, admin_user_id, admin_username, status_code, latency_ms, input_tokens, output_tokens, cache_read_tokens, cache_creation_tokens, stream, billable, usage_estimated, completion_status, request_type, tool_calls)
		VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		log.Protocol, log.Model, log.UpstreamModel, providerID, distributionKeyID, log.DistributionKeyName, consumerUserID, log.ConsumerEmail, adminUserID, log.AdminUsername, log.StatusCode, log.LatencyMS, log.InputTokens, log.OutputTokens, log.CacheReadTokens, log.CacheCreationTokens, boolInt(log.Stream), boolInt(log.Billable), boolInt(log.UsageEstimated), log.CompletionStatus, log.RequestType, log.ToolCalls); err != nil {
		return err
	}
...
func normalizeRequestLog(log RequestLog) RequestLog {
	if !log.BillableSet {
		log.Billable = log.StatusCode >= 200 && log.StatusCode < 300
	}
	if strings.TrimSpace(log.CompletionStatus) == "" {
		if log.StatusCode >= 200 && log.StatusCode < 300 {
			log.CompletionStatus = "completed"
		} else {
			log.CompletionStatus = "upstream_failed"
		}
	}
	if strings.TrimSpace(log.RequestType) == "" {
		log.RequestType = log.Protocol
	}
	return log
}
...
func (s *Store) ConsumerLogsSearch(ctx context.Context, consumerUserID int64, limit, offset int, search string) ([]RequestLog, error) {
	...
	args = append(args, limit, offset)
	rows, err := s.db.QueryContext(ctx, `SELECT l.id, l.protocol, l.model, l.distribution_key_id, COALESCE(NULLIF(l.distribution_key_name, ''), k.name, ''), l.status_code, l.latency_ms, l.input_tokens, l.output_tokens, l.cache_read_tokens, l.cache_creation_tokens, l.stream, l.billable, l.usage_estimated, l.completion_status, l.request_type, l.tool_calls, l.created_at
		FROM request_logs l
		LEFT JOIN distribution_keys k ON k.id = l.distribution_key_id
		`+where+`
		ORDER BY l.created_at DESC, l.id DESC LIMIT ? OFFSET ?`, args...)
	...
	for rows.Next() {
		...
		var distributionKeyID sql.NullInt64
		var createdAt string
		if err := rows.Scan(&l.ID, &l.Protocol, &l.Model, &distributionKeyID, &l.DistributionKeyName, &l.StatusCode, &l.LatencyMS, &l.InputTokens, &l.OutputTokens, &l.CacheReadTokens, &l.CacheCreationTokens, &l.Stream, &l.Billable, &l.UsageEstimated, &l.CompletionStatus, &l.RequestType, &l.ToolCalls, &createdAt); err != nil {
			return nil, err
		}
		l.CreatedAt = parseDBTime(createdAt)
		if l.InputTokens > 0 && l.CacheReadTokens > 0 {
...
// RecoverStaleChatConversationOperation releases an operation only after its lock has expired.
// It is used after a process restart when no in-memory cancellation handle remains.
func (s *Store) RecoverStaleChatConversationOperation(ctx context.Context, owner ChatOwner, id int64, operation, status, statusMessage string, staleAfter time.Duration) (ChatConversation, bool, error) {
	operation = NormalizeChatConversationOperation(operation)
	if operation == "" {
		return ChatConversation{}, false, ErrNotFound
	}
	status = NormalizeChatConversationStatus(status)
	statusMessage = strings.TrimSpace(statusMessage)
	if status == ChatConversationStatusIdle {
	...
	args := []any{status, statusMessage, id}
	args = append(args, ownerArgs...)
	args = append(args, operation, cutoff)
	res, err := s.db.ExecContext(ctx, `UPDATE chat_conversations
		SET active_operation = '', active_operation_started_at = NULL, status = ?, status_message = ?, status_updated_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP
		WHERE id = ? AND `+where+` AND active_operation = ? AND (active_operation_started_at IS NULL OR active_operation_started_at <= ?)`, args...)
	if err != nil {
		return ChatConversation{}, false, err
	}
	affected, err := res.RowsAffected()
...
func (s *Store) DeleteChatConversation(ctx context.Context, owner ChatOwner, id int64) error {
	...
	args = append([]any{id}, args...)
	res, err := s.db.ExecContext(ctx, `DELETE FROM chat_conversations WHERE id = ? AND `+where+` AND COALESCE(active_operation, '') = ''`, args...)
	if err != nil {
		return err
	}
	affected, err := res.RowsAffected()
	...
	if affected == 0 {
		conv, lookupErr := s.ChatConversation(ctx, owner, id)
		if lookupErr != nil {
			return lookupErr
		}
		if conv.ActiveOperation != "" {
			return ErrChatConversationBusy
		}
		return ErrNotFound
	}
	return nil
...
func (s *Store) UpdateChatContextSummary(ctx context.Context, owner ChatOwner, id int64, summary string, throughMessageID int64) (ChatConversation, error) {
	where, args, err := chatOwnerWhere("", owner)
	if err != nil {
		return ChatConversation{}, err
	}
	summary = strings.TrimSpace(summary)
	args = append([]any{summary, throughMessageID, id}, args...)
	res, err := s.db.ExecContext(ctx, `UPDATE chat_conversations
		SET context_summary = ?, context_summary_through_message_id = ?, updated_at = CURRENT_TIMESTAMP
...
func (s *Store) ChatMessages(ctx context.Context, owner ChatOwner, conversationID int64) ([]ChatMessage, error) {
	...
	rows, err := s.db.QueryContext(ctx, `SELECT id, conversation_id, parent_message_id, request_id, role, content, metadata, status, created_at
		FROM chat_messages
	...
	for rows.Next() {
		...
		var createdAt string
		if err := rows.Scan(&msg.ID, &msg.ConversationID, &parentID, &msg.RequestID, &msg.Role, &msg.Content, &msg.Metadata, &msg.Status, &createdAt); err != nil {
			return nil, err
		}
		if parentID.Valid {
			id := parentID.Int64
		...
		msg.Status = NormalizeChatMessageStatus(msg.Status)
...
func (s *Store) chatMessage(ctx context.Context, conversationID, id int64) (ChatMessage, error) {
	...
	var createdAt string
	var parentID sql.NullInt64
	err := s.db.QueryRowContext(ctx, `SELECT id, conversation_id, parent_message_id, request_id, role, content, metadata, status, created_at
		FROM chat_messages
		WHERE id = ? AND conversation_id = ?`, id, conversationID).
		Scan(&msg.ID, &msg.ConversationID, &parentID, &msg.RequestID, &msg.Role, &msg.Content, &msg.Metadata, &msg.Status, &createdAt)
	if errors.Is(err, sql.ErrNoRows) {
		return ChatMessage{}, ErrNotFound
	}
	if err != nil {
...
func (s *Store) CreateChatTurn(ctx context.Context, owner ChatOwner, conversationID int64, content, requestID string) (ChatMessage, ChatMessage, error) {
	...
	defer tx.Rollback()
	userResult, err := tx.ExecContext(ctx, `INSERT INTO chat_messages(conversation_id, role, content, metadata, status) VALUES(?, ?, ?, '{}', ?)`, conversationID, ChatRoleUser, content, ChatMessageStatusCompleted)
	if err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	...
	assistantResult, err := tx.ExecContext(ctx, `INSERT INTO chat_messages(conversation_id, parent_message_id, request_id, role, content, metadata, status) VALUES(?, ?, ?, ?, '', '{}', ?)`, conversationID, userID, requestID, ChatRoleAssistant, ChatMessageStatusGenerating)
	if err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	assistantID, err := assistantResult.LastInsertId()
	if err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	if _, err := tx.ExecContext(ctx, `UPDATE chat_conversations SET last_message_at = CURRENT_TIMESTAMP, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, conversationID); err != nil {
	...
	user, err := s.chatMessage(ctx, conversationID, userID)
	if err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	assistant, err := s.chatMessage(ctx, conversationID, assistantID)
	return user, assistant, err
}

func (s *Store) ChatMessageByRequestID(ctx context.Context, owner ChatOwner, conversationID int64, requestID string) (ChatMessage, error) {
	if _, err := s.ChatConversation(ctx, owner, conversationID); err != nil {
		return ChatMessage{}, err
	}
	var id int64
	err := s.db.QueryRowContext(ctx, `SELECT id FROM chat_messages WHERE conversation_id = ? AND request_id = ?`, conversationID, strings.TrimSpace(requestID)).Scan(&id)
	if errors.Is(err, sql.ErrNoRows) {
		return ChatMessage{}, ErrNotFound
	}
	if err != nil {
...
func (s *Store) UpdateChatAssistant(ctx context.Context, owner ChatOwner, conversationID, messageID int64, content, metadata, status string) (ChatMessage, error) {
	if _, err := s.ChatConversation(ctx, owner, conversationID); err != nil {
		return ChatMessage{}, err
	}
	status = NormalizeChatMessageStatus(status)
	if strings.TrimSpace(metadata) == "" {
		metadata = "{}"
	}
	res, err := s.db.ExecContext(ctx, `UPDATE chat_messages SET content = ?, metadata = ?, status = ? WHERE id = ? AND conversation_id = ? AND role = ?`, content, metadata, status, messageID, conversationID, ChatRoleAssistant)
	if err != nil {
		return ChatMessage{}, err
	}
	affected, err := res.RowsAffected()
	...
	return s.chatMessage(ctx, conversationID, messageID)
}

func (s *Store) ResetLatestChatAssistant(ctx context.Context, owner ChatOwner, conversationID, messageID int64, requestID string) (ChatMessage, ChatMessage, error) {
	if _, err := s.ChatConversation(ctx, owner, conversationID); err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	tx, err := s.db.BeginTx(ctx, nil)
	...
	defer tx.Rollback()
	var latestID, parentID int64
	var role string
	err = tx.QueryRowContext(ctx, `SELECT id, role, COALESCE(parent_message_id, 0) FROM chat_messages WHERE conversation_id = ? ORDER BY id DESC LIMIT 1`, conversationID).Scan(&latestID, &role, &parentID)
	if errors.Is(err, sql.ErrNoRows) || latestID != messageID || role != ChatRoleAssistant || parentID == 0 {
		return ChatMessage{}, ChatMessage{}, ErrNotFound
	}
	if err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	var parentRole string
	if err := tx.QueryRowContext(ctx, `SELECT role FROM chat_messages WHERE id = ? AND conversation_id = ?`, parentID, conversationID).Scan(&parentRole); err != nil || parentRole != ChatRoleUser {
		if errors.Is(err, sql.ErrNoRows) || parentRole != ChatRoleUser {
			return ChatMessage{}, ChatMessage{}, ErrNotFound
		}
		return ChatMessage{}, ChatMessage{}, err
	}
	if _, err := tx.ExecContext(ctx, `UPDATE chat_messages SET request_id = ?, content = '', metadata = '{}', status = ? WHERE id = ?`, strings.TrimSpace(requestID), ChatMessageStatusGenerating, messageID); err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	if _, err := tx.ExecContext(ctx, `UPDATE chat_conversations SET context_summary = CASE WHEN context_summary_through_message_id >= ? THEN '' ELSE context_summary END, context_summary_through_message_id = CASE WHEN context_summary_through_message_id >= ? THEN 0 ELSE context_summary_through_message_id END, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, messageID, messageID, conversationID); err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	if err := tx.Commit(); err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	parent, err := s.chatMessage(ctx, conversationID, parentID)
	if err != nil {
		return ChatMessage{}, ChatMessage{}, err
	}
	assistant, err := s.chatMessage(ctx, conversationID, messageID)
	return parent, assistant, err
}
...
func NormalizeChatMessageStatus(status string) string {
	switch strings.TrimSpace(status) {
	case ChatMessageStatusGenerating:
		return ChatMessageStatusGenerating
	case ChatMessageStatusFailed:
		return ChatMessageStatusFailed
